    ...
    void Update()
    {
        switch (GameWave)
        {
            ...
            case すごろく.Move:
                ...
                if ((Vector3.Distance(pointB, mypos) > 0.05f && !finished) && saikoro != 0)
                {
                    ...
                    else
                    {
                        ...
                        int startValue = 0;
                        if ((!pointDatas[nowPoint].isLower && !reverseWinding) || (!pointDatas[nowPoint - 1].isLower && reverseWinding))//単位円の右側か左側か
                        {
                            if (right)
                            {
                                startValue = 1;
